...
class CharField(Field):
    ...
    def __init__(self, required: bool = False, nullable: bool = False, max_length: int = None):
    ...
    def validate(self, value: Any) -> Optional[str]:
        ...
        if not isinstance(value, str):
            raise TypeError(f"'{self.field_name}' must be a string, got {type(value).__name__}")

        if self.max_length and len(value) > self.max_length:
            raise ValueError(f"'{self.field_name}' cannot be longer than {self.max_length} characters")
...
class ArgumentsField(Field):
    ...
    def validate(self, value: Any) -> Optional[Dict]:
        ...
        if not isinstance(value, dict):
            raise TypeError(f"'{self.field_name}' must be a dictionary, got {type(value).__name__}")
...
class PhoneField(Field):
    ...
    def validate(self, value: Any) -> Optional[str]:
        ...
        if not isinstance(value, str):
            raise TypeError(f"'{self.field_name}' must be a string or number, got {type(value).__name__}")

        # Remove all non-digit characters
        cleaned = re.sub(r'\D', '', value)

        if len(cleaned) != 11:
            raise ValueError(f"'{self.field_name}' must contain exactly 11 digits, got {len(cleaned)}")
    ...
    def validate(self, value: Any) -> Optional[int]:
        ...
        if not isinstance(value, int):
            try:
                ...
                raise TypeError(f"'{self.field_name}' must be a number, got {type(value).__name__}")
...
class ClientIDsField(Field):
    ...
    Validates that value is a non-empty list of integers.
    """
    ...
    def validate(self, value: Any) -> Optional[List[int]]:
        ...
        if not isinstance(value, list):
            raise TypeError(f"'{self.field_name}' must be a list, got {type(value).__name__}")
        ...
        for idx, item in enumerate(value):
            try:
                ...
                raise TypeError(
                                f"'{self.field_name}' must contain only numbers")
...
class OnlineScoreRequest(BaseRequest):
    ...
    def is_valid(self) -> bool:
        """
        Validate the request with additional business rules.

        In addition to base field validation, ensures at least one
        required pair of fields is provided.
        ...
        # Check at least one pair of fields is present
        has_phone_email = (self.phone is not None or self.email is not None) and (self.phone or self.email)
        has_name = self.first_name and self.last_name
        has_gender_birthday = (self.gender is not None and self.birthday is not None)

        if not (has_phone_email or has_name or has_gender_birthday):
            self._errors['_validation'] = (
        ...
        if request:
            ...
            if path in self.router:
                try:
                    ...
                    code = INTERNAL_ERROR
                    response = {"error": ERRORS.get(INTERNAL_ERROR, "Internal Server Error")}
            else:
                code = NOT_FOUND
...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--port", action="store", type=int, default=8080, help="Port to listen on")
    parser.add_argument("-l", "--log", action="store", default=None, help="Log file path (default: stdout)")
